...
A demanda para este sistema foi validada inicialmente através de entrevistas exploratórias com especialistas acadêmicos, que relataram a desconexão crônica entre a documentação de engenharia e a logística de canteiro. 

Esses usuários (pequenos empreiteiros, mestres de obra e proprietários) não possuem acesso a sistemas ERP complexos devido ao alto custo e à elevada complexidade de implantação.

**Evidência de interesse e cenário de mercado:**
O setor da construção civil no Brasil é marcado por altos índices de desperdício de materiais, frequentemente atrelados a falhas na etapa de suprimentos e à falta de digitalização nos canteiros de obras de pequeno porte. O sistema proposto atua exatamente nessa lacuna, servindo como uma ponte digital prática entre a documentação técnica (projeto/memorial) e a execução operacional da obra, mitigando a compra errônea ou excessiva de insumos.
...
### 2.7 Riscos de Integração e Alternativas

| Integração | Risco | Alternativa |
| :--- | :--- | :--- |
| **SINAPI (Catálogo de Preços)** | Alteração no formato das planilhas mensais fornecidas pela Caixa ou indisponibilidade do portal. | A atualização de preços ocorre de forma assíncrona no banco de dados. Caso o portal falhe, o sistema continua operando com a última tabela válida salva no banco. O usuário também mantém a liberdade de inserir preços manualmente na lista de compras. |
...
**Mecânica de Integração com o SINAPI:**
A obtenção das estimativas de custo não ocorrerá via requisições síncronas a serviços externos, garantindo baixa latência na resposta ao usuário. O sistema utilizará uma rotina de *ETL (Extract, Transform, Load)* executada periodicamente pelo `Node-cron`. Este *worker* será responsável por acessar os catálogos mensais em formato CSV disponibilizados pela Caixa Econômica, aplicar tratamentos de sanitização nos dados (como a normalização de caracteres e padronização de códigos de produtos) e persistir os valores atualizados na base de dados PostgreSQL. Dessa forma, a listagem de compras apenas consulta a base de dados interna, garantindo resiliência e alta performance.
